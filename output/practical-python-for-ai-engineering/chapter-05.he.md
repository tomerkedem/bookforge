...
כמעט כל פרויקט פייתון מתחיל עם קובץ אחד: main.py, לפעמים app.py, ובימים פחות ממושמעים גם script_final_v2_fixed.py.

בהתחלה זה נוח. 
הכול מרוכז, אין צורך לחשוב על מבנה, ופשוט מתקדמים.

זה עובד מצוין כל עוד מדובר בניסוי קטן. 
אבל בשלב מסוים, בלי נקודת מעבר ברורה, הקובץ מתחיל לגדול. לוגיקה עסקית מתערבבת עם קוד תשתיתי, פונקציות מתחילות להיות תלויות זו בזו, ושינוי נקודתי כבר לא מרגיש מקומי.

בנקודה הזו אתה לא מתמודד עם קוד שלא עובד, אלא עם קוד שקשה לשלוט בו.

כשהכול נמצא בקובץ אחד:

- אין הפרדה אמיתית בין אחריות לרכיבים

- coupling גבוה בין חלקים במערכת

- קשה לבודד לוגיקה ולכתוב בדיקות ממוקדות

- שינוי קטן עלול להשפיע על אזורים לא צפויים

- עבודה בצוות הופכת למסורבלת ומלאת התנגשויות

הבעיה כאן היא לא כמות הקוד, אלא היעדר מבנה.

קוד שלא מחולק למודולים הוא קוד בלי גבולות ברורים. 
אין שכבות, אין חוזים, ואין דרך לגדול בלי לשלם מחיר בכל שינוי.

הפתרון הוא לא לכתוב פחות קוד, אלא לארגן אותו נכון. 
זו בדיוק הסיבה שפייתון בנויה סביב מודולים וחבילות. לא כדי “לסדר קבצים”, אלא כדי לאפשר חשיבה הנדסית שמחזיקה לאורך זמן.
...
בפייתון, כל קובץ הוא גם **מודו**ל וגם **תוכנית בפני עצמ**ה. כשאתה מריץ קובץ ישירות, משתנה פנימי בשם ‎__name__‎ מקבל את הערך "__main__". אבל כשקובץ מיובא כמודול ממקום אחר, הוא יקבל את שמו האמיתי, לדוגמה "text_utils". וזה בדיוק מה שמאפשר להפריד בין **קוד להרצ**ה לבין **קוד לשימוש חוזר.**
