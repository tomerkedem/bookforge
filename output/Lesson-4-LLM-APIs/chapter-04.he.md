...
**עכשיו נעבור לחלק החשוב באמת: עבודה עם קוד אמיתי.**

עד עכשיו דיברנו על AI כמושג. 
מכאן מתחילים לעבוד איתו כמו מהנדסי תוכנה.

נפתח פרויקט חדש ונתחיל לבנות צעד אחר צעד סביבת עבודה בסיסית שמתקשרת ישירות עם מודלי Anthropic דרך API. לאורך נכתב את כל הקבצים ונראה כיצד מערכת Python פשוטה יכולה להתחיל לעבוד מול מודל שפה אמיתי.
...
המטרה של החלק הזה אינה רק "להצליח לשלוח Prompt".
...
בעולם האמיתי אנחנו כמעט אף פעם לא עובדים לבד.
...
אסור לכתוב API Keys ישירות בתוך קוד המקור.
...
לאחר שסיימנו להקים את סביבת העבודה ולהתקין את הספריות הדרושות, הוא עבר לרגע המרכזי הראשון של החלק המעשי: שליחת בקשת API אמיתית למודל של Anthropic.
...
בלי SDK היינו צריכים לבנות לבד:
...
**אחת מנקודות האבטחה החשובות ביותר:**
...
בנוסף לשמירת ה-API Key בקובץ env, חייבים לעדכן גם את קובץ gitignore.
...
זהו למעשה שלב ה-Authentication מול ספק ה-AI.
...
אפשר לחשוב עליו כעל "שער הכניסה" לעולם ה-API.
...
בפועל מאחורי הקלעים מתרחש תהליך שלם:
...
Haiku הוא מודל קטן ומהיר יחסית, שמתאים במיוחד:
...
טוקנים הם המשאב המרכזי בעולם ה-LLMs:
...
בערך 0 המודל הופך להיות הרבה יותר יציב ודטרמיניסטי.
...
מודלים מודרניים עובדים במבנה של Messages ולא Prompt יחיד.
...
אחד המנגנונים החשובים ביותר במערכות AI מודרניות: Streaming.

לצורך ההדגמה ניצור את הקובץ:
...
קריאת API רגילה האפליקציה שולחת בקשה וממתינה עד שכל התשובה מוכנה.
...
ברגע שהמשתמש מחכה יותר מדי זמן בלי לראות תגובה, המערכת מתחילה להרגיש:
...
מאחורי הקלעים זה עובד בצורה שונה לחלוטין.
...
text_stream הוא Generator שמחזיר חלקים קטנים של טקסט בזמן אמת.
...
מאחורי הקלעים מתרחש תהליך רציף:
...
אחת היכולות המעניינות ביותר במודלי Claude המודרניים: מצב Thinking.
...
בקובץ הזה נעבור לשלב מתקדם יותר:
...
זה מעניין משום שגם מודלים מהירים יחסית מתחילים לקבל יכולות reasoning מתקדמות יותר.
...
שאלה שאינה רק ניסוח טקסטואלי פשוט, אלא דורשת:
...
זו נקודה חשובה מאוד בעולם ה-AI Engineering.
...
התגובה כאן אינה רק String פשוט.
...
אחד השימושים החשובים ביותר של מודלי LLM בעולם האמיתי:
...
זה בדיוק המקום שבו LLMs הופכים לכלי עוצמתי במיוחד.
...
JSON הוא אחד הפורמטים החשובים ביותר בעולם ה-AI וה-APIs.
...
זו דוגמה קלאסית למשימת Information Extraction.
...
JSON Schema הוא למעשה "חוזה" שמגדיר למודל:
