...
אחרי שבחרנו את השלדים שנבנה בשיעור, אפשר להתחיל מהשלד הראשון: RAG Chatbot.

לפני שהצ’אטבוט יכול לענות על שאלות מתוך מסמכים, צריך להכין עבורו מאגר ידע שניתן לחפש בו לפי משמעות. זה התפקיד של בסיס ה-RAG.

בחלק הזה נבנה את שכבת ההכנה של ה-RAG: ניקח טקסט רגיל, נטען אותו מתוך קבצים, נחלק אותו ל-chunks, נהפוך כל chunk ל-embedding, ונשמור את הכול בתוך Vector Store.
